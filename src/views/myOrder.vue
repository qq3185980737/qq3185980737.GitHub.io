...
console.log(this.list2)
    ...
    var b2= window.sessionStorage.getItem("userlx");
      loginAxios("/ttbiaodan2",this.kj,JSON.stringify(this.mark11),b1,b2).then(result=>{
          if (result.data.code==9000){     
          this.Visible1=false
          this.Visible2=false
            this.$message.succuss("victory");
...
var ok=[{"field":"工号","value":"100001"},{"field":"密码","value":""},{"field":"姓名","value":""},{"field":"身份","value":""},{"field":"领导","value":""}]
      ok[0].value=this.user.username
loginAxios("/query",this.user.tag,JSON.stringify(ok)).then(result=>{
          if (result.data.code==9000){     
            this.Visible3=false
...
this.columnsss.push({
        ...
        operator: [{
          ...
          click: (row) => {
            ...
            window.sessionStorage.setItem("userll",a);//转换为字符
            window.sessionStorage.setItem("userlx",b);//转换为字符
            loginAxios("/adtable",this.user.tag).then(result=>{
          if (result.data.code===9000){  
            console.log(result.data.data)
this.formrule1=result.data.data
this.Visible2=true
